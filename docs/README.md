...
This repository demonstrates the use of C# Generic through a simple console application. In this repository, a generic class is designed to create multiple quiz variations, each with different data types for the `question` and `answer` variables. By using generic, we can create multiple quiz variations without needing separate classes for each variation, which reduces code duplication, enhances reusability, and ensures type safety.
